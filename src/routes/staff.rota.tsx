...
export function RotaPage() {
  ...
  const monthlyGridDays = useMemo(() => {
    // Exact days of the current month.
    ...
    <div className="max-w-[1600px] mx-auto space-y-4 md:space-y-6 pb-20 font-sans">
      ...
      <div className="bg-white p-4 md:p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-3 w-full xl:w-auto">
          ...
          <div className="flex flex-row items-center gap-2 w-full md:w-auto justify-between md:justify-start">
            ...
            <div className="bg-slate-100 p-1 rounded-xl flex shrink-0">
              {(['DAILY', 'WEEKLY', 'MONTHLY'] as const).map(v => (
                ...
                >
                  {v.substring(0, 1)}<span className="hidden sm:inline">{v.substring(1).toLowerCase()}</span>
                </button>
              ))}
            </div>

            {/* Date Selector (Expands on Mobile) */}
            <div className="flex items-center bg-white border border-slate-200 rounded-xl shadow-sm flex-1 md:flex-none justify-between">
               ...
               <span className="px-2 text-[10px] md:text-[11px] font-black uppercase tracking-widest text-slate-700 w-full md:w-36 text-center truncate">
          ...
          {isManager && (
            <div className="grid grid-cols-2 md:flex gap-2 md:pl-2 md:border-l md:border-slate-200 mt-2 md:mt-0 w-full md:w-auto">
              <button onClick={() => setModalState('SHIFT')} className="w-full flex items-center justify-center gap-1.5 px-2 md:px-4 py-2.5 md:py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-black text-[9px] md:text-[10px] uppercase tracking-widest rounded-xl transition-all border border-indigo-200 shadow-sm">
                <Plus size={14} /> Add Shift
              </button>
              <button onClick={() => setModalState('LEAVE')} className="w-full flex items-center justify-center gap-1.5 px-2 md:px-4 py-2.5 md:py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 font-black text-[9px] md:text-[10px] uppercase tracking-widest rounded-xl transition-all border border-rose-200 shadow-sm">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm h-[calc(100vh-16rem)] min-h-[600px] flex flex-col relative overflow-hidden">
        ...
        {view === 'MONTHLY' ? (
          <div className="flex-1 flex flex-col bg-slate-50 min-h-0">
            ...
            <div className="grid grid-cols-7 flex-1 auto-rows-max overflow-y-auto content-start">
              {monthlyGridDays.map((date, i) => {
                ...
                // Calculate the grid column offset for the 1st day of the month
                // date.getDay() returns 0 for Sunday, 1 for Monday, etc.
                // We want Monday=1, Sunday=7 for CSS grid-column-start
                let colStartClass = '';
                if (i === 0) {
                   const jsDay = date.getDay();
                   const cssCol = jsDay === 0 ? 7 : jsDay;
                   // Dynamic classes must be complete strings for Tailwind to detect them during build.
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            {/* Header Row (Mobile Optimized Widths) */}
            <div className="flex border-b border-slate-200 bg-slate-50 shadow-sm shrink-0">
              <div className="w-24 sm:w-32 md:w-64 p-3 md:p-4 border-r border-slate-200 flex items-center shrink-0">
                <span className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest">Staff Member</span>
              </div>
                  ...
                  {virtualItems.map((virtualRow) => {
                    ...
                    return (
                      <div key={staff.id} className="flex border-b border-slate-100 hover:bg-slate-50/50 transition-colors h-16 group" style={{ height: `${virtualRow.size}px` }}>
                        ...
                        {/* Name Column (Mobile Optimized Width & Font) */}
                        <div className="w-24 sm:w-32 md:w-64 p-2 md:p-4 border-r border-slate-200 flex flex-col justify-center bg-white shrink-0">
                           <div className="flex items-center gap-2">
                             <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${staff.is_active === false ? 'bg-rose-500' : 'bg-emerald-500'}`} />
                             <p className="text-[10px] md:text-sm font-bold text-slate-900 truncate">{staff.name}</p>
                           </div>
                           <p className="text-[8px] md:text-[10px] text-slate-400 font-mono mt-0.5 uppercase ml-3.5 truncate">{staff.role.replace('_', ' ')}</p>
                        </div>
                        
                        {/* Shifts Cells (Mobile Optimized Font) */}
                        <div className="flex-1 flex min-w-0">
                           {matrixDays.map(date => {
                             const dateKey = format(date, 'yyyy-MM-dd');
                             ...
                             return (
                               <div key={date.toString()} className="flex-1 border-r border-slate-100 last:border-0 p-1 md:p-1.5 relative group/cell min-w-[70px] flex items-center justify-center">
                                  {leave ? (
                                    <div className="w-full h-full rounded-lg bg-rose-50 border border-rose-200 flex flex-col items-center justify-center p-1 text-center">
                                  ...
                                  <div className="w-full p-2 rounded-xl bg-white border border-slate-200 shadow-sm flex flex-col items-center justify-center text-center relative group/cell hover:border-indigo-300 hover:shadow-md transition-all">
                                    <span className="text-[10px] font-black text-slate-900 tracking-tight flex items-center gap-1">
                                      <Clock size={10} className="text-slate-400" /> Working
                                    </span>
                                    {shift.assigned_area && (
